...
function updateXDomain(data, x, time) {
  ...
  time == "Days" ? x.domain([d3.time.day.offset(new Date(), -30), d3.time.day.offset(new Date(), -14)]) :
...
function setXDomain(time, w, margin) {
  ...
  time == "Days" ? d3.time.scale().domain([d3.time.day.offset(new Date(), -30), d3.time.day.offset(new Date(), -14)]).rangeRound([0, w - margin.left - margin.right]) :
...
function barMouseOver(d, i, x, y, bar, tooltip, user_id, time, metric, barWidth) {
  ...
  if (d.load_volume != 0) {
    ...
    $(".graph-tooltip").append(toolTipContents(d, user_id, metric));
  }
}
...
function toolTipMouseOver(tooltip) {
  $(".graph-tooltip").mouseenter(function() {
      clearTimeout($(".graph-tooltip").attr("d"));
...
function toolTipContents(d, user_id, metric) {
  ...
  if (parseInt(+d.load_volume) > 0) {
    tooltip_contents.push(metric + ": " + d.load_volume);
  }
...
$(document).on('ready page:load', function () {	
	...
	var view = "water_usage";

	if ($(".usage-section").length > 0) { draw(900, 360, margin, 25, 1, view); }		
}); 
